...
function buildHistoryPreviewHtml(item, timestamp, escapeHtmlFn, buildSnippetFn) {
  const escape = typeof escapeHtmlFn === "function" ? escapeHtmlFn : (v) => String(v || "");
  const snippet = typeof buildSnippetFn === "function"
    ? buildSnippetFn
    : (text, maxLen = 80) => {
  ...
  const promptSource = item?.promptText || item?.prompt || "";
  const answerSource = item?.answerText || item?.answer || "";
  const promptPreview = snippet(promptSource, 80);
  const answerPreview = snippet(answerSource, 110);
  ...
  return `
      <div class="history-preview">
        ...
        <div style="font-size: 12px; color: var(--color-text-secondary); margin-bottom: 8px; white-space: pre-wrap;">${escape(answerPreview || "No response available")}</div>
...
function buildHistoryDetailHtml(item, timestamp, escapeHtmlFn) {
  const escape = typeof escapeHtmlFn === "function" ? escapeHtmlFn : (v) => String(v || "");
  const promptText = item?.promptText || item?.prompt || "";
  return `
    <div style="margin-bottom: 20px;">
      ...
      <div style="font-size: 13px; color: var(--color-text); margin-bottom: 16px; white-space: pre-wrap; background: var(--color-bg); padding: 16px; border-radius: 8px; line-height: 1.6;">${escape(promptText)}</div>
